...
# analysis globals: variables, their attributes and ComBat expression -------
  ...
  ## functional group and association with BCR risk
  
  net$variables <- globals$genes
  
  net$attr_tbl$gene_group <- globals$genes_lexicon %>% 
    set_names(c('variable', 'gene_group'))
  
  net$attr_tbl$bcr <- uni_cut$common_significant %>% 
    map(~tibble(variable = .x)) %>% 
    compress(names_to = 'bcr_risk')
  
  net$attr_tbl <- net$attr_tbl %>% 
    reduce(left_join, by = 'variable') %>% 
    mutate(bcr_risk = ifelse(is.na(bcr_risk), 'ns', bcr_risk), 
           bcr_risk = factor(bcr_risk, c('unfavorable', 'favorable', 'ns')))
  ...
  net$attr_lexicon <- 
    list(community_id = c('community_id', 'gene communities'), 
         ...
         bcr_risk = c('bcr_risk', 'BCR marker')) 
...
# Network plots ---------
  ...
  for(i in net$attr_lexicon) {
    ...
    if(i[[1]] == 'bcr_risk') vtx_shape_var <- 'bcr_risk'
    ...
    net$graph_plots[[i[[1]]]] <- 
      ...
      pmap(plot, 
           ...
           vertex_shape_variable = vtx_shape_var, 
           vertex_color = 'black', 
           edge_alpha = 0.4, 
           label_vertices = TRUE, 
           vertex_txt_color_variable = i[[1]], 
           vertex_txt_size = 2.25, 
  ...
  ## BCR risk: shape and fill scale
  
  net$graph_plots$bcr_risk <- net$graph_plots$bcr_risk %>% 
    map(~.x + 
          ...
          scale_color_manual(values = c(unfavorable = 'firebrick', 
                             ...
                             drop = FALSE, 
                             name = 'association with risk') + 
          scale_shape_manual(values = c(unfavorable = 23, 
                                        favorable = 22, 
                                        ns = 21), 
                             drop = FALSE, 
                             name = 'association with risk'))
